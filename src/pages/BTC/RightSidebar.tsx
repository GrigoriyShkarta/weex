...
const RightSidebar: React.FC = () => {
	...
	const { data, isPending } = useQuery({
		...
		queryFn: async () => {
			const response = await axios.get(
				'https://data-api.coindesk.com/spot/v1/latest/tick?market=coinbase&instruments=BTC-USD,ETH-USD,DOGE-USD&apply_mapping=true&groups=VALUE&api_key=6a872e0423bc7b53e6013b797175842b4005304d35cf581d2d6b08bac82da82a&browser_cache_busting=1744187646919'
			)
	...
	const calculateWxt = () => {
		...
		const coinPrice = data['BTC-USD'].PRICE
		const calculated = usdAmount / coinPrice
	...
	return (
		<div className='w-[296px] max-sm:w-full max-sm:mt-[14.9vw]'>
			{/* Converter Section */}
			<div className=''>
				<h2 className='text-[18px] font-[500]'>Sign Up to Buy Bitcoin</h2>
			...
			<div className='mt-[56px]'>
				<div className='flex items-center gap-[16px]'>
					...
					/>
					<h2 className='text-[18px] font-[700]'>Bitcoin Price</h2>
				</div>
				...
				<div className='flex items-center flex-wrap mt-[16px] gap-[29px]'>
					...
					{/* <div className='flex items-center text-green-500'>
						...
						+4.84%
						<span className='text-gray-500 ml-1 text-sm'>(24h)</span>
					</div> */}
				</div>

				<p className='mt-[16px] p-[16px] text-[14px] rounded-[16px] border border-[#282828] text-[#d1d1d2]'>
					The current Bitcoin price is{' '}
					<span className='font-bold'>${data['BTC-USD'].PRICE}</span>, with a
					24-hour trading volume of <span className='font-bold'>$9.32M</span>.
					Bitcoinhas <span className='font-bold text-green-500'>increased</span>{' '}
					by <span className='font-bold'>4.84</span>% in the past 24 hours.
					Bitcoin market cap currently sits at{' '}
